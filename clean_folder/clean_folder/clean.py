...
DSP_INFO = {"base_path": "", "max_file_name_len": 0}
...
def output_info_print():
    # Output to console
    if DSP_INFO["max_file_name_len"]:
        max_file_name_len = max(DSP_INFO["max_file_name_len"], MAX_PRINT_STR_LEN)
...
def file_action(file: Path):
    if len(file.name) > DSP_INFO["max_file_name_len"]:
        DSP_INFO["max_file_name_len"] = len(file.name)
...
def known_files_sorting(file: Path, destination_folder: str):
    ...
    try:
        ...
        file.replace(Path(DSP_INFO["base_path"], destination_folder, normalize(file.stem) + file.suffix))
...
def unknown_files_sorting(file: Path, destination_folder: str):
    ...
    try:
        ...
        file.replace(Path(DSP_INFO["base_path"], destination_folder, file.name))
        FILE_TYPES[destination_folder]["file_list"].append(file.name)
    except OSError as err:
        error_message_print(file, err)
...
def archive_unpack(file: Path, destination_folder: str):
    ...
    try:
        ...
        shutil.unpack_archive(str(file),
                              str(Path(DSP_INFO["base_path"], destination_folder, normalize(file.stem))))
        file.unlink()
        FILE_TYPES[destination_folder]["file_list"].append(file.name)
    except OSError as err:
        error_message_print(file, err)
...
def clean_folder():
    ...
    DSP_INFO["base_path"] = base_path_dir
